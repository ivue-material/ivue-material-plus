<template>
  <div>
    ...
    <h1>上传状态</h1>
    <p>
      通过 status 属性可以标识上传状态，uploading 表示上传中，failed
      表示上传失败，done 表示上传完成。
    </p>
    ...
    <div>
      ...
      <p>
        ...
        属性中传入一个函数，在函数中通过 file.type 区分文件类型，返回 true
        表示超出限制，false 表示未超出限制。
      </p>
      <ivue-upload
        v-model="fileList2"
        multiple
...
export default {
  ...
  methods: {
    isOverSize(file) {
      const maxSize = file.type === 'image/jpeg' ? 100 * 1024 : 1000 * 1024;
