...
export default defineComponent({
  ...
  setup(props, { slots }) {
    // form inject
    const formContext = inject(FormContextKey, undefined) as FormContext;
    // 父级item
    const parentFormItemContext = inject(
      FormItemContextKey,
      undefined
    ) as FormItemContext;
    ...
    const normalizedRules = computed(() => {
      ...
      if (formRules && props.prop) {
        // 获取当前item对应的规则
        const currentRules = getProp(formRules, props.prop).value;
