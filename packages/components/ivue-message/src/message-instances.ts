...
type Type = 'normal' | 'info' | 'warning' | 'success' | 'error' | 'loading';
...
const message = (type: Type, options: Options) => {
  // 自定义id
  const customizeId = options.id as string;
...
const close = (id: string, userOnClose?: (vm: VNode) => void): void => {
  ...
  if (notificationsId[id]) {
    // 隐藏自定义的通知
    (
      orientedNotifications[idx].vm.component!.proxy as MessageInstance
    ).visible = false;
...
const closeAll = (): void => {
  ...
  for (const key in instances) {
    instances.forEach(({ vm }) => {
      ...
      (vm.component!.proxy as MessageInstance).visible = false;
    });
  }
};
